...
function script_check_duplicate($name = false, $timeout_mins = 5 ) {
    ...
    usleep(500);
    if (mx_config_get($name . "_pid") != getmypid()) {
...
function http($url)
{
    ...
    if (g('http_cache')) {
        ...
        if ($data) {
            ...
            $base = inside("href='", "'", str_replace('"', "'", inside("<base", ">", $http_html)));
            if (!$base) {
                $base = $url;
                if (strrpos($base, '.')>strrpos($base, '/')) $base = substr($base, 0, strrpos($base, '/')+1);
            }
            $base = if_inside("", "?", $base);
            $base = if_inside("", "&", $base);
            $base = if_inside("", "#", $base);
...
function http_curl($url, $post = false, $file = false)
{
    ...
    if (!$user_agent) $user_agent = 'Mozilla/5.0 (compatible; Baiduspider/2.0; +http://www.baidu.com/search/spider.html)';
    ...
    global $http_cookies;
    if ((substr($http_cookies, 0, 1)!='+') and ($http_cookies!==false) and ($http_cookies!==NULL)) curl_setopt($ch, CURLOPT_COOKIE, $http_cookies);
    else {
        if (substr($http_cookies, 0, 1)=='+') curl_setopt($ch, CURLOPT_COOKIE, substr($http_cookies,1));
    ...
    if ($http_code == 429 && !$http_proxy) {
        xnotice("HTTP Result ".$http_code." - ". http_code_message());
        global $mx_curl_save_header;
        xlog("<b>Response headers:</b> ".$mx_curl_save_header);
        xlog($http_html);
        xlog("Pausing for 30 sec");
        sleep(30);
    }
...
function mx_debug_fetch_result($http_html) {
    ...
    $id++;
    $m = "<a class='debug_toggle' onclick='document.getElementById(\"fetch$id\").hidden = ! document.getElementById(\"fetch$id\").hidden'>
        [ Fetch Result ]</a>
        <div id='fetch$id' hidden='hidden'>";
    if (substr($http_html,0,4)=="http")
        $m .= "<img src='$http_html' />";
...
function if_inside ($start, $end="", $source = false) {
...
function replace($search=false, $replace='', $source = "#NULL")
{
    if ($source === "#NULL") { global $http_html; $source = $http_html; } // Use set_source() default
        ...
        if (!$search) {
            ...
            if (($p=strpos($search,'*'))!==false) {
                ...
                if (strpos($replace,'*')!==false) {
                    $r = $source;
                    $p = 0;
                    while ( ($p = stripos($r, $before, $p)) !== false) {
                        $p2 = stripos($r, $after, $p + strlen($before));
                        if (!$p2) break;
                        $star = substr($r, $p + strlen($before), $p2 - $p - strlen($before));
                        $new = str_replace('*', $star, $replace);
                        $r = substr($r, 0, $p) . $new . substr($r, $p2 + strlen($after));
                        $p += strlen($new);
                    }
                } else {
...
function replace_inside($find, $replace, $start, $end, $source = "#NULL") {
...
function tags_text($selector, $source = false)
{
    ...
    foreach (nodes($selector, $source) as $node) {
        ...
        if ($text=="") $text = $node->GetAttribute('value');
        if ($text!=="") $r[] = $text;
    }
...
function tags_html($selector, $source = false)
{
    ...
    foreach ($noko->get($selector)->getNodes() as $tag) {
        ...
        //Iframe workaround
        $p = 0;
        while (($p = stripos($html, '<iframe', $p)) !== false) {
            $p = strpos($html, '>', $p+1);
            if ($html[$p-1]!='/') continue;
            $html = substr($html, 0, $p-1).'></iframe'.substr($html, $p);
        }
...
function re($return = '[no return value]') {
    ...
    if (!$debug_nesting_level or !ini_get('display_errors') or !error_reporting()) return $return;
...
{
    if (isset($GLOBALS[$global_variable_name]))
...
function var_to_string($var) {
    ...
    if (is_string($var)) {
        ...
        if ($var[0] == "[") $var = $literals[0].strpos($literals, "[").substr($var,1);
        return $var;
    }
    if (!is_array($var)) {
        trigger_error ("Unsupported variable type in var_to_string");
        return "";
    }
    $i = 0;
    $res = array();
    foreach($var as $index=>$value) {
        if (is_string($value)) {
            for($k=1; $k<strlen($literals); $k++)
                $value = str_replace($literals[$k], $literals[0].$k, $value);
        } else $value = var_to_string($value);
        if ($index===$i) {
            $i++;
            $res[] = $value;
        } else $res[] = $index.":".$value;
    }
    $res = "[".implode(",", $res)."]";
    return $res;
}

function string_to_var($string) {
    ...
    if ($string[0]=="[") { // Array
        if (substr($string, -1) != "]") {
            trigger_error ("No closing ']' in string_to_varr");
            return "";
        }
        $res = array();
        $s = substr($string, 1, -1);
        ...
        while ($s) {
            $comma = strpos($s, ",");
            if ($comma === false) $comma = strlen($s);
            $bracket = strpos($s, "[");
            if ($bracket === false) $bracket = strlen($s);
            $dots = strpos($s, ":");

            if ($dots && $dots < $comma && $dots < $bracket) {
                $index = substr($s, 0, $dots);
                $s = substr($s, $dots+1);
                $comma -= $dots+1;
            } else {
                $index = $i;
            ...
            if (substr($s, 0, 1)=="[") { // Find closing bracket
                $n=0; $p=0;
                while ($p<strlen($s)) {
                    if ($s[$p]=="[") $n++;
                    elseif ($s[$p]=="]") {
                ...
                $res[$index] = string_to_var(substr($s, 0, $p+1));
                $s = substr($s, $p+1);
                if (substr($s, 0, 1)==",") $s = substr($s, 1);
            } else {
                $value = substr($s, 0, $comma);
                for($k=1; $k<strlen($literals); $k++)
                    $value = str_replace($literals[0].$k, $literals[$k], $value);
                $res[$index] = $value;
                $s = substr($s, $comma+1);
            }
        }
    ...
    if (substr($string, 0, 2) == $literals[0].strpos($literals, "[")) $string = "[".substr($string,2);
    return $string;
}